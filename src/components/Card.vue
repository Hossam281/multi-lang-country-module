<template>
    <div class="w-full p-3 overflow-clip  flex h-[400px] flex-col gap-4 text-white bg-[#393E46] rounded-lg shadow-xl shadow-[#00adb5] ">
        ...
        <h4 class="text-xl font-mono">Population: {{ country.population ? country.population : "No population provided" }}</h4>
        <h4 class="text-xl font-mono">capital: {{ country.capital ? country.capital : "No capital provided" }}</h4>
        <p class="text-md overflow-clip font-mono h-[70%]">{{ country.details ? country.details : "No details provided" }}</p>
